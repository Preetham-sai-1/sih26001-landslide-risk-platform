"""
V10 Model Training & Validation Pipeline.

Trains Monotone XGBoost on Train + Validation splits (< 2018):
- Includes all 34 validated features + V10 operational risk dynamics
...
- Saves model package to ml-service/models/model_v10.joblib
- Generates ml-service/reports/v10_training_report.md
...
def run_v10_training():
    print("=================================================================")
    print("      EXECUTING V10 MODEL TRAINING & VALIDATION PIPELINE         ")
    print("=================================================================")
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(df_train, df_train["target"], groups)):
        ...
        clf.fit(d_tr[ALL_V10_FEATURES], d_tr["target"])
        raw_oof_preds[val_idx] = clf.predict_proba(d_val[ALL_V10_FEATURES])[:, 1]
    ...
    # 3. FIT FINAL V10 MODEL ON FULL TRAIN SPLIT (< 2018)
    print("\nPhase 3: Fitting Final V10 Model on Full Train Split (< 2018)...")
    clf_v10 = xgb.XGBClassifier(
        n_estimators=100,
        max_depth=4,
        learning_rate=0.05,
        subsample=0.8,
    ...
    clf_v10.fit(df_train[ALL_V10_FEATURES], df_train["target"])
    ...
    for r in rain_sweep:
        ...
        p_raw = clf_v10.predict_proba(row_test[ALL_V10_FEATURES])[0, 1]
    ...
    MODEL_V10_PATH.parent.mkdir(parents=True, exist_ok=True)
    ...
    model_package = {
        ...
        "features": ALL_V10_FEATURES
    }
    joblib.dump(model_package, MODEL_V10_PATH)
    print(f"Saved model package to {MODEL_V10_PATH}")

    val_report_md = f"""# V10 Model Training & Validation Report (Train + Validation Splits)
...
- **Features Included**: {len(ALL_V10_FEATURES)} total (34 validated features + 8 operational risk dynamics)
...
"""
    ...
    with open(REPORTS_DIR / "v10_training_report.md", "w") as f:
        f.write(val_report_md)

    print(f"Saved training report to {REPORTS_DIR / 'v10_training_report.md'}")
