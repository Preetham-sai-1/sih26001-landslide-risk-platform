"""
V9 Model Training & Validation Pipeline.
...
- Saves model package to ml-service/models/model_v9.joblib
- Generates ml-service/reports/v9_validation_report.md
...
def run_v9_training():
    print("=================================================================")
    print("      EXECUTING V9 MODEL TRAINING & VALIDATION PIPELINE          ")
    print("=================================================================")
    ...
    constraints = get_monotone_constraints(ALL_VALIDATED_FEATURES)
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(df_train, df_train["target"], groups)):
        ...
        clf.fit(d_tr[ALL_VALIDATED_FEATURES], d_tr["target"])
        raw_oof_preds[val_idx] = clf.predict_proba(d_val[ALL_VALIDATED_FEATURES])[:, 1]
    ...
    # 3. FIT FINAL V9 MODEL ON FULL TRAIN SPLIT (< 2018)
    print("\nPhase 3: Fitting Final V9 Model on Full Train Split (< 2018)...")
    clf_v9 = xgb.XGBClassifier(
        n_estimators=100,
        max_depth=4,
        learning_rate=0.05,
        subsample=0.8,
    ...
    clf_v9.fit(df_train[ALL_VALIDATED_FEATURES], df_train["target"])
    ...
    for r in rain_sweep:
        ...
        p_raw = clf_v9.predict_proba(row_test[ALL_VALIDATED_FEATURES])[0, 1]
    ...
    # 5. SAVE MODEL PACKAGE & VALIDATION REPORT
    ...
    joblib.dump(model_package, MODEL_V9_PATH)
    print(f"Saved model package to {MODEL_V9_PATH}")

    val_report_md = f"""# V9 Model Validation Evaluation Report (Train + Validation Splits)
...
"""
    ...
    with open(REPORTS_DIR / "v9_validation_report.md", "w") as f:
        f.write(val_report_md)

    print(f"Saved validation report to {REPORTS_DIR / 'v9_validation_report.md'}")
